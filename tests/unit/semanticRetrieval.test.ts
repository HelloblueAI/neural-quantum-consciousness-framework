...
import type { LearnedEmbeddingProvider } from '../../src/core/TensorLogicEngineEnhancements';
import {
  cosineSimilarity,
  rankTextsByOverlap,
  rankTextsWithProvider,
  SEMANTIC_RETRIEVAL_EVAL,
} from '../../src/lab/semanticRetrieval';
  ...
  it('ranks with a learned embedding provider', async () => {
    const provider: LearnedEmbeddingProvider = {
      ...
      generateBatchEmbeddings: async (texts: string[]) =>
        texts.map((text) => (text.includes('Tehran') || text.includes('Iran') ? [1, 0] : [0, 1])),
    };
