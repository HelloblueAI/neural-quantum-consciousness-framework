...
export function buildCapabilitiesEndpointPayload(
  ...
  return {
    ...
    measured: true,
    disclaimer:
      "Capability scores are derived from learning engine state — not consciousness simulation.",
    timestamp: Date.now(),
    capabilities: {
      reasoningQuality: capabilities.reasoningQuality,
      systemDepth: capabilities.systemDepth,
...
/** Back-compat aliases for cached dashboard JS reading /status (measured data only). */
export function withLegacyStatusShims(
  payload: ReturnType<typeof buildLabStatusPayload>,
) {
  return {
    ...payload,
