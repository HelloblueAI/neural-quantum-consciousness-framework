...
import { MemorySystem } from "@/reasoning/MemorySystem";
    ...
    switch (task.id) {
      ...
      case "memory-recall": {
        ...
        return {
          ...
          score: passed ? 1 : 0,
          message: passed
            ? "Memory retrieved matching context"
            : "No matching memory",
          durationMs: Date.now() - start,
        };
      }
      ...
      case "semantic-retrieval": {
        ...
        const top = ranked[0];
        const passed =
          top?.text === SEMANTIC_RETRIEVAL_EVAL.expectedTop &&
          (top?.score ?? 0) > 0;
        return {
          id: task.id,
          name: task.name,
