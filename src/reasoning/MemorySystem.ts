/**
 * Memory System
 * Persistent memory for the reasoning system using in-memory storage (KV-ready)
 */
...
export class MemorySystem {
  ...
  public getStats() {
    return {
      ...
      byType: {
        fact: Array.from(this.memories.values()).filter(
          (m) => m.type === "fact",
        ).length,
        concept: Array.from(this.memories.values()).filter(
          (m) => m.type === "concept",
        ).length,
        experience: Array.from(this.memories.values()).filter(
          (m) => m.type === "experience",
        ).length,
        learning: Array.from(this.memories.values()).filter(
          (m) => m.type === "learning",
        ).length,
      },
  ...
  public export(): string {
    return JSON.stringify({
      ...
      knowledgeGraph: Array.from(this.knowledgeGraph.entries()).map(
  ...
  public import(data: string): void {
    try {
      ...
      this.knowledgeGraph = new Map(
        parsed.knowledgeGraph.map(([key, value]: [string, string[]]) => [
          key,
          new Set(value),
        ]),
      );
