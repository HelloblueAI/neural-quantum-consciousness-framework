...
export class FuzzyLogic extends EventEmitter {
  ...
  private membershipFunctions: Map<string, any> = new Map();
  ...
  public async infer(premises: any[]): Promise<any[]> {
    try {
  ...
  private fuzzifyInput(input: any): any {
    // Convert crisp input to fuzzy sets
    const fuzzified: any = {};
    
    // Fuzzify goals
    if (input.goals) {
      fuzzified.goals = input.goals.map((goal: any) => ({
        original: goal,
    ...
    if (input.constraints) {
      fuzzified.constraints = input.constraints.map((constraint: any) => ({
        original: constraint,
  ...
  private calculateMembership(value: number, functionName: string): number {
  ...
  private calculateTriangularMembership(value: number, params: number[]): number {
    const [a, b, c] = params;
  ...
  private calculateTrapezoidalMembership(value: number, params: number[]): number {
  ...
  private calculateGaussianMembership(value: number, params: number[]): number {
    const [mean, std] = params;
    if (mean === undefined || std === undefined || std === 0) return 0;
    return Math.exp(-Math.pow(value - mean, 2) / (2 * Math.pow(std, 2)));
  }
  
  private applyFuzzyRules(fuzzifiedInput: any): any[] {
    const conclusions: any[] = [];
  ...
  private applyGoalRules(goals: any[]): any[] {
    const conclusions: any[] = [];
  ...
  private applyConstraintRules(constraints: any[]): any[] {
    const conclusions: any[] = [];
  ...
  private applyContextRules(context: any): any[] {
    const conclusions: any[] = [];
  ...
  private defuzzifyResults(fuzzyConclusions: any[]): any[] {
  ...
  private calculateFuzzyConfidence(results: any[]): number {
  ...
  private calculateUncertainty(results: any[]): any {
  ...
  private generateFuzzySteps(fuzzifiedInput: any, fuzzyConclusions: any[]): any[] {
    const steps: any[] = [];
  ...
  private applyFuzzyInference(fuzzifiedPremise: any): any {
