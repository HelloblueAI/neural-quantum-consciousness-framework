...
export class Logger extends EventEmitter {
  ...
  public log(
    ...
    try {
      ...
      console.log(
        `[${level.toUpperCase()}] ${this.config.component}: ${message}`,
        data,
      );
    }
  }
  ...
  public getHistory(level?: LogLevel, limit?: number): LogEntry[] {
    ...
    if (level) {
      filtered = filtered.filter(
        (entry) => Logger.LOG_LEVELS[entry.level] >= Logger.LOG_LEVELS[level],
      );
    }
  ...
  public getPerformanceMetrics(): any {
    return {
      ...
      errorRate:
        ...
        Math.max(this.performanceMetrics.totalLogs, 1),
      warningRate:
  ...
  public exportLogs(format: "json" | "text" = "json"): string {
    if (format === "json") {
      ...
      return this.logHistory
        .map(
          (entry) =>
            `[${new Date(entry.timestamp).toISOString()}] [${entry.level.toUpperCase()}] ${entry.component}: ${entry.message}`,
        )
        .join("\n");
    }
  }
  ...
  private addToHistory(entry: LogEntry): void {
    ...
    if (this.logHistory.length > this.config.maxEntries) {
      this.logHistory.splice(
        0,
        this.logHistory.length - this.config.maxEntries,
      );
    }
  }
  ...
  private analyzeErrorPatterns(): any {
    const errors = this.logHistory.filter(
  ...
  private analyzePerformanceTrends(): any {
    const performanceEntries = this.logHistory.filter(
      (entry) => entry.performance,
    );

    return {
      averageLogTime: this.performanceMetrics.averageLogTime,
      memoryUsage: performanceEntries.map(
        (entry) => entry.performance?.memoryUsage || 0,
      ),
      cpuUsage: performanceEntries.map(
        (entry) => entry.performance?.cpuUsage || 0,
      ),
    };
  }
  ...
  private analyzeComponentUsage(): any {
    ...
    for (const entry of this.logHistory) {
      componentUsage[entry.component] =
        (componentUsage[entry.component] || 0) + 1;
    }
  ...
  private calculateErrorFrequency(errors: LogEntry[]): number {
    ...
    const timeSpan =
      (this.logHistory[this.logHistory.length - 1]?.timestamp || 0) -
      (this.logHistory[0]?.timestamp || 0);
